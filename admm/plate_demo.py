...
idxs = np.random.randint(4000, size=1)
print idxs
# result folder
...
for idx in idxs :
	...
	x_shape = ori_img.shape        
	(A_fun, AT_fun, mask, mask_not) = problem.setup(ori_img,x_shape)
        y = ori_img #A_fun(ori_img)
	#y = A_fun(grnd_img)	
	y_art = A_fun(grnd_img) + mask_not
	#y, noise = add_noise.exe(A_fun(ori_img), noise_mean=noise_mean, noise_std=noise_std)
	...
	if 1:
        ...
        filename = '%s/mask.jpg' % base_folder
        sp.misc.imsave(filename, sp.misc.imresize((reshape_img(np.clip(mask, 0.0, 1.0))*255).astype(np.uint8), 4.0, interp='nearest'))
	...
	if 1:
            ...
            plt.subplot(1,5,2)
            plt.imshow(reshape_img_fun(y), interpolation='nearest')
            plt.title('y actual, SNR=%2.2f' % (psnr_bad)) 
            plt.subplot(1,5,3)
            plt.imshow(reshape_img_fun(255*mask), interpolation='nearest')
            plt.title('mask extracted A()')
            plt.subplot(1,5,4)
            plt.imshow(reshape_img_fun(y_art), interpolation='nearest')
            plt.title('y = A(ground truth)')
            plt.subplot(1,5,5)
            plt.imshow(img)
